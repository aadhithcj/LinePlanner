...
/**
 * Color mapping for machine type badges
 */
...
/**
 * Animated panel showing selected machine details
 */
export const MachineInfoPanel = () => {
  const { selectedMachine, setSelectedMachine, ui } = useLineStore();
  
  if (!selectedMachine) return null;
  
  const { operation } = selectedMachine;
  const machineCategory = getMachineCategory(operation.machine_type);
  const badgeColor = MACHINE_BADGE_COLORS[machineCategory] || MACHINE_BADGE_COLORS.default;
  
  return (
    <AnimatePresence>
      {ui.isMachineInfoOpen && (
        <motion.div
          ...
          <div className="bg-gradient-to-r from-primary/20 to-accent/20 p-4 border-b border-border/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-semibold text-foreground">Operation #{operation.op_no}</h3>
          ...
          <div className="p-4 space-y-4">
            ...
            {operation.op_name && (
              ...
              >
                ...
                <p className="text-foreground font-medium mt-1">{operation.op_name}</p>
              </motion.div>
            )}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-foreground font-semibold text-lg mt-1">
                  {operation.smv.toFixed(2)} <span className="text-sm text-muted-foreground font-normal">min</span>
                </p>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-foreground font-medium mt-1">{operation.section}</p>
              </div>
            </motion.div>
          </div>
